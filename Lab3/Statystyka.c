...
void calculate_cdf(double cdf[], int trials) {
    ...
    for ( int i = 0; i < trials; i++ ) {
        ...
        for ( int j = a + b - 2; j < TWO_DICE_SUM; j++)
            cdf[j]++;

    }

    for ( int i = 0; i < TWO_DICE_SUM; i++ )
        cdf[i] = cdf[i] / trials * 100;
}
...
void monty_hall(int *wins, double *win_probability, int n) {
	...
	*wins = 0;
    ...
    for(int i = 0; i < n; i++) {
		...
		int remaining = ( choice + 1 ) % 3; // your second choice (if you switch)
        if ( remaining == reveal ) remaining = ( remaining + 1 ) % 3;
